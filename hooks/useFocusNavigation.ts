...
export function useFocusNavigation({
  ...
  const navigateUp = useCallback(() => {
    if (focusMode === "paragraph") return; // already at coarsest
  ...
  const navigateDown = useCallback(() => {
    if (focusMode === "word") return; // already at finest
    ...
    if (nextMode === "word" && lastWordRange) {
      ...
      // Find the word boundary closest to the saved range
      ...
      // Find which new boundary contains the start of the current range
      const currentRange = boundaries[currentIndex];
      if (currentRange) {
        const newBounds = getBoundaries(content, nextMode);
        const newIdx = newBounds.findIndex(
  ...
  const handleFocusKeyDown = useCallback(
    (e: KeyboardEvent): boolean => {
      ...
      // Don't intercept if modifier keys are held (except shift for level change)
  ...
  const focusNavState: FocusNavState = useMemo(
    () => ({
      mode: focusMode,
      focusedRange,
      lastFocusedWordRange:
        focusMode !== "word" ? lastWordRange : null,
      isNavigating,
    }),
