...
const App: React.FC = () => {
  ...
  return (
    <div 
      className="w-screen h-screen flex flex-col relative overflow-hidden transition-colors duration-500 font-mono"
    ...
    >
      ...
      <div className="absolute top-8 right-8 z-50 flex gap-4">
        {THEMES.map(t => (
            <button
                ...
                className={`w-6 h-6 rounded-full border border-black/10 shadow-sm transition-all duration-300 ${themeId === t.id ? 'scale-125 ring-2 ring-offset-2' : 'hover:scale-110'}`}
                style={{ 
      ...
      {/* Main Area */}
      <main className="flex-1 w-full h-full relative z-10 pt-10 transition-all duration-300 ease-in-out">
        {viewMode === 'write' ? (
            <Typewriter 
                content={content}
            ...
            <div 
              className="mx-auto h-full relative z-10 transition-[max-width] duration-300 ease-in-out" 
      ...
      {/* Bottom Toolbar Area */}
      <footer className="absolute bottom-0 left-0 right-0 p-8 flex justify-between items-end z-50 pointer-events-none">
        
        {/* Left: Interactive Tools */}
        <div className="flex flex-col gap-4 pointer-events-auto">
          
          {/* Syntax Toggles Row */}
          <div className="flex gap-2 mb-2 p-2 bg-black/5 rounded-lg backdrop-blur-sm w-fit transition-opacity duration-300" style={{ opacity: viewMode === 'write' ? 1 : 0 }}>
             <button onClick={() => toggleHighlight('nouns')} 
                     className={`p-2 rounded hover:bg-white/50 transition-all ${highlightConfig.nouns ? 'opacity-100 scale-110' : 'opacity-40 grayscale'}`}
                     style={{ color: currentTheme.highlight.noun }}
                     title="Highlight Nouns">
                <IconNoun />
             </button>
             <button onClick={() => toggleHighlight('verbs')} 
                     className={`p-2 rounded hover:bg-white/50 transition-all ${highlightConfig.verbs ? 'opacity-100 scale-110' : 'opacity-40 grayscale'}`}
                     style={{ color: currentTheme.highlight.verb }}
                     title="Highlight Verbs">
                <IconVerb />
             </button>
             <button onClick={() => toggleHighlight('adjectives')} 
                     className={`p-2 rounded hover:bg-white/50 transition-all ${highlightConfig.adjectives ? 'opacity-100 scale-110' : 'opacity-40 grayscale'}`}
                     style={{ color: currentTheme.highlight.adjective }}
                     title="Highlight Adjectives">
                <IconAdj />
             </button>
             <button onClick={() => toggleHighlight('conjunctions')} 
                     className={`p-2 rounded hover:bg-white/50 transition-all ${highlightConfig.conjunctions ? 'opacity-100 scale-110' : 'opacity-40 grayscale'}`}
          ...
          <div className="flex gap-6 items-center">
            ...
            <div className="relative flex items-center group">
              ...
              {showWidthControl && (
                <div className="absolute left-full ml-4 p-2 bg-white/90 shadow-lg rounded-lg backdrop-blur-sm flex items-center border border-black/5 animate-in fade-in slide-in-from-left-2 duration-200">
                  <input 
                    type="range" 
                    min="400" 
                    max="1400" 
        ...
        <div className="pointer-events-auto bg-black/5 px-4 py-2 rounded-lg backdrop-blur-sm">
            <span className="text-3xl font-bold font-mono tracking-tighter" style={{ color: currentTheme.text }}>
              {wordCount}
            </span>
            <span className="text-xs uppercase tracking-widest ml-2 opacity-50">words</span>
        </div>
