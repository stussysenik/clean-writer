import { SongAnalysis, SongLine, SongWord, RhymeGroup, FlowMetrics, RhymeScheme } from "../types";
...
function countSyllablesCmu(phonemes: string): number {
  return phonemes.split(" ").filter((p) => CMU_VOWELS.test(p)).length || 1;
}
...
export function getRhymeKey(
  ...
  for (let i = lower.length - 1; i >= 0; i--) {
    if (vowels.includes(lower[i])) {
      lastVowelStart = i;
      while (lastVowelStart > 0 && vowels.includes(lower[lastVowelStart - 1])) {
        lastVowelStart--;
      }
      break;
    }
...
export function detectRhymeScheme(lines: SongLine[]): RhymeScheme {
  ...
  let nextLetter = 0;
  const pattern = endRhymes.map((key) => {
  ...
  const counts: Record<string, number> = {
    monorhyme: 0, couplets: 0, alternating: 0, enclosed: 0, free: 0,
  };

  const windowSize = Math.min(4, pattern.length);
  const numWindows = Math.max(1, pattern.length - windowSize + 1);
  ...
  const totalNonFree = counts.monorhyme + counts.couplets + counts.alternating + counts.enclosed;
  if (counts.free > totalNonFree || numWindows === 0) {
...
export function computeFlowMetrics(
  ...
  rhymeMap: Map<string, Set<string>>,
): FlowMetrics {
  ...
  for (const line of lines) {
    ...
    for (const word of line.words) {
      ...
      lineRhymeKeys.set(word.rhymeKey, (lineRhymeKeys.get(word.rhymeKey) || 0) + 1);
    }
  ...
  const rhymeDensity = totalWords > 0 ? Math.round((rhymingWords / totalWords) * 100) : 0;
  const avgSyllablesPerLine = nonEmptyLines > 0
    ? Math.round((totalSyllables / nonEmptyLines) * 10) / 10
    : 0;

  const multiSyllabicRhymes = rhymeGroups.filter((g) => g.key.length >= 3).length;
 ...
 */
export function analyzeSong(text: string, cmuDict?: CmuDict): SongAnalysis {
  ...
  const flowMetrics = computeFlowMetrics(lines, rhymeGroups, rhymeMap);

  return { lines, rhymeGroups, totalSyllables, nonLatinWarning, flowMetrics, rhymeScheme };
}
