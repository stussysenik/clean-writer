...
test.describe("Core Writing Mechanics", () => {
  ...
  test.describe("Strikethrough", () => {
    ...
    test("strikethrough can be typed manually with ~~ markers", async ({
      page,
    }) => {
      const textarea = page.locator("textarea");
    ...
    test("strikethrough renders with line-through styling", async ({
      page,
    }) => {
      const textarea = page.locator("textarea");
      await textarea.click();
      ...
      const textDecoration = await strikeSpan.evaluate(
        (el) => getComputedStyle(el).textDecoration,
      );
      expect(textDecoration).toContain("line-through");
    });

    test("strikethrough button can be clicked after selecting text", async ({
      page,
    }) => {
      const textarea = page.locator("textarea");
    ...
    test("clean action removes all complete strikethrough segments", async ({
      page,
    }) => {
      await page.evaluate(() => {
