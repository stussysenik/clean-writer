...
test.describe("Responsive Layout", () => {
  test.describe("Mobile viewport", () => {
    ...
    test("uses smaller font size on mobile", async ({ page }) => {
      ...
      const backdrop = page
        .locator('div[class*="whitespace-pre-wrap"]')
        .first();
      const fontSize = await backdrop.evaluate(
  ...
  test.describe("Content reflow", () => {
    ...
    test("text wraps properly at different widths", async ({ page }) => {
      ...
      const longText =
        "This is a very long sentence that should wrap properly across multiple lines when the viewport is narrow enough to require text wrapping behavior.";
