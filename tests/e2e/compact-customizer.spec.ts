...
test.describe('Compact Customizer', () => {
  ...
  test('customizer scrollHeight ≤ 1.5× clientHeight on mobile viewport', async ({ page }) => {
    // Set mobile viewport
    ...
    expect(measurements.scrollHeight).toBeLessThanOrEqual(measurements.clientHeight * 1.5);
  });

  test('"Visible Presets" is the first section heading after "Customize Theme"', async ({ page }) => {
    // Open customizer
    const gearBtn = page.locator('button[title="Customize Theme"]');
    await gearBtn.click();
    await expect(page.locator('h2:has-text("Customize Theme")')).toBeVisible();
  ...
  test('font section collapsed by default, expands on click', async ({ page }) => {
    // Open customizer
    const gearBtn = page.locator('button[title="Customize Theme"]');
    await gearBtn.click();
    await expect(page.locator('h2:has-text("Customize Theme")')).toBeVisible();
