...
test.describe("Mobile Paradigm", () => {
  ...
  test.describe("Touch Target Sizes", () => {
    ...
    test("primary interactive buttons meet minimum touch target", async ({
      page,
    }) => {
  ...
  test.describe("Fold-Tab Behavior", () => {
    test("fold-tab shows word count when closed", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("one two three", { delay: 10 });
      await page.waitForTimeout(600);
    ...
    test("fold-tab click opens panel", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("test content", { delay: 10 });
      await page.waitForTimeout(600);
  ...
  test.describe("Touch Interactions", () => {
    ...
    test("strikethrough uses persisted selection after blur", async ({
      page,
    }) => {
      const textarea = page.locator("textarea");
      const strikeBtn = page.locator('[data-testid="strikethrough-btn"]');
      const persistedOverlay = page.locator(
    ...
    test("no double-tap zoom on interactive elements", async ({ page }) => {
      ...
      const foldTab = page.locator('[data-testid="mobile-fold-tab"]');
      const touchAction = await foldTab.evaluate(
  ...
  test.describe("Focus Mode", () => {
    test("mobile taps retarget the highlighted sentence without showing the desktop anchor", async ({
      ...
      expect(initialCaretColor).not.toBe("rgba(0, 0, 0, 0)");
      expect(initialCaretColor).not.toBe("transparent");
      ...
      expect(caretColorAfterTap).not.toBe("rgba(0, 0, 0, 0)");
      expect(caretColorAfterTap).not.toBe("transparent");
    });
  });
  ...
  test.describe("Panel Content on Mobile", () => {
    ...
    test("panel shows word count", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("one two three four five", { delay: 10 });
      await page.waitForTimeout(600);
    ...
    test("highlight toggles work in mobile panel", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("The quick fox", { delay: 10 });
      await page.waitForTimeout(600);
  ...
  test.describe("Viewport and Layout", () => {
    ...
    test("toolbar is accessible", async ({ page }) => {
      const toolbar = page
        .locator('[data-testid="strikethrough-btn"]')
        .locator("..");
  ...
  test.describe("Different Mobile Sizes", () => {
    test("works on iPhone SE (small)", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("small screen test", { delay: 10 });
      await page.waitForTimeout(600);
    ...
    test("works on iPad Mini (tablet)", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("tablet test", { delay: 10 });
      await page.waitForTimeout(600);
    ...
    test("works in landscape orientation", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("landscape test", { delay: 10 });
      await page.waitForTimeout(600);
  ...
  test.describe("Discovery Animation", () => {
    test("fold-tab pulses for first-time users", async ({ page }) => {
      ...
      const hasAnimation = await foldTab.evaluate(
        (el) =>
  ...
  test.describe("Accessibility on Mobile", () => {
    ...
    test("panel can be navigated with focus", async ({ page }) => {
      ...
      // Can receive focus
      await foldTab.focus();
      const isFocused = await foldTab.evaluate(
