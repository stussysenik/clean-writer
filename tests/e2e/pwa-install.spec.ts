...
test.describe('PWA Support', () => {
  test('manifest.webmanifest is served', async ({ page }) => {
    ...
    expect(manifest.name).toBe('Clean Writer');
  ...
  test('PWA icons are accessible', async ({ page }) => {
    ...
    for (const icon of icons) {
      ...
      expect(response?.status(), `${icon} should be accessible`).toBe(200);
  ...
  test('apple-mobile-web-app-title meta tag is set', async ({ page }) => {
    ...
    expect(title).toBe('Clean Writer');
  });
  ...
  test('service worker registers in production build', async ({ page }) => {
    ...
    test.skip(true, 'Service worker tests require production build');
  });
});
