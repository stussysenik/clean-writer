...
test.describe("Responsive Syntax Panel", () => {
  ...
  test.describe("Desktop (>= 1024px)", () => {
    ...
    test("word count is displayed in desktop panel", async ({ page }) => {
      ...
      const wordCount = page.locator(
        '[data-testid="desktop-syntax-panel"] .text-5xl',
      );
  ...
  test.describe("Mobile (< 1024px)", () => {
    ...
    test("dragging fold tab opens panel through harmonica stages", async ({
      page,
    }) => {
      const foldTab = page.locator('[data-testid="mobile-fold-tab"]');
  ...
  test.describe("Paradigm Transition", () => {
    test("transitions smoothly when resizing from mobile to desktop", async ({
      page,
    }) => {
    ...
    test("transitions smoothly when resizing from desktop to mobile", async ({
      page,
    }) => {
  ...
  test.describe("Hover Effects", () => {
    ...
    test("hovering syntax category highlights words in text", async ({
      page,
    }) => {
      ...
      const foxSpan = page
        .locator('.whitespace-pre-wrap span:has-text("fox")')
        .first();
  ...
  test.describe("Garfield Cursor", () => {
    ...
    test("cursor color changes based on last typed word syntax", async ({
      page,
    }) => {
  ...
  test.describe("Harmonica Gesture", () => {
    ...
    test("drag snaps back if released before 50% threshold", async ({
      page,
    }) => {
      const foldTab = page.locator('[data-testid="mobile-fold-tab"]');
      const box = await foldTab.boundingBox();
    ...
    test("tap on tab does not toggle (drag-only interaction)", async ({
      page,
    }) => {
      const foldTab = page.locator('[data-testid="mobile-fold-tab"]');
    ...
    test("shows directional arrow affordance on closed tab", async ({
      page,
    }) => {
