...
test.describe("Responsive Paradigm - Mutual Exclusivity", () => {
  ...
  async function setupViewportAndLoad(
    page: any,
    width: number,
    height: number,
  ) {
  ...
  test.describe("Desktop Mode (>= 1024px)", () => {
    test("Desktop: left panel visible at 1024px (exact breakpoint)", async ({
      page,
    }) => {
    ...
    test("Desktop: NO right fold-tab at 1024px", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="desktop-syntax-panel"]'),
  ...
  test.describe("Mobile Mode (< 1024px)", () => {
    test("Mobile: right fold-tab visible at 1023px (just below breakpoint)", async ({
      page,
    }) => {
    ...
    test("Mobile: right fold-tab visible at 375px (iPhone)", async ({
      page,
    }) => {
    ...
    test("Mobile: right fold-tab visible at 768px (tablet)", async ({
      page,
    }) => {
  ...
  test.describe("Breakpoint Transition", () => {
    test("transition from mobile to desktop at exact breakpoint", async ({
      page,
    }) => {
      // Start at mobile
      await setupViewportAndLoad(page, 1023, 768);
    ...
    test("transition from desktop to mobile at exact breakpoint", async ({
      page,
    }) => {
      // Start at desktop
      await setupViewportAndLoad(page, 1024, 768);
    ...
    test("rapid resize does not cause both to appear", async ({ page }) => {
      ...
      const textarea = page.locator("textarea");
      await textarea.click();
      await textarea.pressSequentially("test content for transition", {
        delay: 10,
      });
      await page.waitForTimeout(800);
  ...
  test.describe("Content Preservation Across Transitions", () => {
    test("content preserved when transitioning from mobile to desktop", async ({
      page,
    }) => {
      // Start mobile
    ...
    test("content preserved when transitioning from desktop to mobile", async ({
      page,
    }) => {
      // Start desktop
      await page.setViewportSize({ width: 1280, height: 800 });
      ...
      const textarea = page.locator("textarea");
      await textarea.click();
      await textarea.pressSequentially("Desktop to mobile content", {
        delay: 10,
      });
      await page.waitForTimeout(100);
  ...
  test.describe("Syntax Panel State Across Transitions", () => {
    test("word count accurate after transition", async ({ page }) => {
      ...
      const textarea = page.locator("textarea");
      await textarea.click();
      await textarea.pressSequentially("one two three four five", {
        delay: 10,
      });
      await page.waitForTimeout(800);
  ...
  test.describe("No Leaking Between Paradigms", () => {
    test("desktop panel styles do not leak to mobile", async ({ page }) => {
      ...
      const leftPositioned = page.locator(
