...
test.describe("State Persistence", () => {
  ...
  test.describe("Content Persistence", () => {
    test("content persists across page reload", async ({ page }) => {
      ...
      await textarea.pressSequentially("This is persistent content", {
        delay: 10,
      });
    ...
    test("strikethrough content persists correctly", async ({ page }) => {
      ...
      await textarea.click();
      await textarea.pressSequentially("~~strikethrough~~ and normal", {
        delay: 10,
      });
      await page.waitForTimeout(500);
  ...
  test.describe("Font Size Persistence", () => {
    test("font size setting persists across reload", async ({ page }) => {
      ...
      const textarea = page.locator("textarea");
      const initialFontSize = await textarea.evaluate(
      ...
      const finalFontSize = await textarea.evaluate(
        (el) => getComputedStyle(el).fontSize,
      );
  ...
  test.describe("Highlight Config Persistence", () => {
    ...
    test("highlight toggles persist across reload", async ({ page }) => {
      ...
      const nounsToggle = panel
        .locator("text=Nouns")
        .locator("..")
        .locator('input[type="checkbox"], [role="switch"], button')
        .first();
  ...
  test.describe("localStorage Keys", () => {
    test("uses correct localStorage keys", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("content for storage test", { delay: 10 });
      await page.waitForTimeout(500);
    ...
    test("localStorage data is valid JSON where applicable", async ({
      page,
    }) => {
      await page.locator("textarea").click();
      await page
  ...
  test.describe("Clear Functionality", () => {
    ...
    test("clear button is clickable", async ({ page }) => {
      ...
      await page
        .locator("textarea")
        .pressSequentially("Content", { delay: 10 });
      await page.waitForTimeout(200);
  ...
  test.describe("Session Recovery", () => {
    test("recovers gracefully from corrupted localStorage", async ({
      page,
    }) => {
      // Set corrupted data
      await page.evaluate(() => {
