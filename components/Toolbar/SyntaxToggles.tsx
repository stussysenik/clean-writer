...
const TOGGLE_CONFIG: ToggleButtonConfig[] = [
  ...
  { key: 'nouns', number: 1, icon: IconNoun, colorKey: 'noun', label: 'Nouns', group: 'content' },
  { key: 'verbs', number: 2, icon: IconVerb, colorKey: 'verb', label: 'Verbs', group: 'content' },
  { key: 'adjectives', number: 3, icon: IconAdj, colorKey: 'adjective', label: 'Adjectives', group: 'content' },
  { key: 'adverbs', number: 4, icon: IconAdverb, colorKey: 'adverb', label: 'Adverbs', group: 'content' },
  // Function Words (5-9) - structural
  { key: 'pronouns', number: 5, icon: IconPronoun, colorKey: 'pronoun', label: 'Pronouns', group: 'function' },
  { key: 'prepositions', number: 6, icon: IconPreposition, colorKey: 'preposition', label: 'Prepositions', group: 'function' },
  { key: 'conjunctions', number: 7, icon: IconConj, colorKey: 'conjunction', label: 'Conjunctions', group: 'function' },
  { key: 'articles', number: 8, icon: IconArticle, colorKey: 'article', label: 'Articles', group: 'function' },
  { key: 'interjections', number: 9, icon: IconInterjection, colorKey: 'interjection', label: 'Interjections', group: 'function' },
];
...
const SyntaxToggles: React.FC<SyntaxTogglesProps> = ({
  ...
  const handleContextMenu = (e: React.MouseEvent, key: keyof HighlightConfig) => {
    e.preventDefault();
    handleDoubleClick(key);
  };

  const renderToggleButton = ({ key, number, icon: Icon, colorKey, label }: ToggleButtonConfig) => {
    const isSoloed = soloMode === key;
    const isDimmedBySolo = soloMode !== null && soloMode !== key;
    const isActive = highlightConfig[key];

    return (
      <Tooltip key={key} content={`${label}${soloMode ? '' : ' (dbl-click to solo)'}`} shortcut={String(number)}>
        <TouchButton
          ...
          style={{
            '--tw-ring-color': isSoloed ? theme.highlight[colorKey] : undefined,
            '--tw-ring-offset-color': theme.background,
          } as React.CSSProperties}
          title={`${label} (${number})`}
        >
          <span style={{ color: theme.highlight[colorKey] }}>
            <Icon />
          </span>
          <span
